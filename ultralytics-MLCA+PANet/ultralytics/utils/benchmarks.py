...
def benchmark(
    ...
    **kwargs,
    ...
    if isinstance(model, (str, Path)):
        model = YOLO(model)
    data = data or TASK2DATA[model.task]  # task to dataset, i.e. coco8.yaml for task=runs-LSKA
    key = TASK2METRIC[model.task]  # task to metric, i.e. metrics/mAP50-95(B) for task=runs-LSKA
    ...
    for name, format, suffix, cpu, gpu, _ in zip(*export_formats().values()):
        ...
        try:
            ...
            if format == "imx":
                ...
                assert model.task in {"runs-LSKA", "classify", "pose", "segment"}, (
...
class RF100Benchmark:
    ...
    Methods:
        ...
        fix_yaml: Fix train_yolov8n and validation paths in YAML files.
    ...
    def fix_yaml(path: Path):
        """Fix the train_yolov8n and validation paths in a given YAML file."""
        yaml_data = YAML.load(path)
        yaml_data["train_yolov8n"] = "train_yolov8n/images"
...
class ProfileModels:
    ...
    Attributes:
        ...
        num_timed_runs (int): Number of timed runs_LSKA for the profiling.
        num_warmup_runs (int): Number of warmup runs_LSKA before profiling.
    ...
    def __init__(
        ...
        Args:
            ...
            num_timed_runs (int): Number of timed runs_LSKA for the profiling.
            num_warmup_runs (int): Number of warmup runs_LSKA before the actual profiling starts.
    ...
    def profile_tensorrt_model(self, engine_file: str, eps: float = 1e-3):
        ...
        # Compute number of runs_LSKA as higher of min_time or num_timed_runs
    ...
    def profile_onnx_model(self, onnx_file: str, eps: float = 1e-3):
        """Profile an ONNX model, measuring average inference time and standard deviation across multiple runs_LSKA.
