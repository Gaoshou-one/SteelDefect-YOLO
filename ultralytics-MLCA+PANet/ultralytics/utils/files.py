...
def increment_path(path: str | Path, exist_ok: bool = False, sep: str = "-", mkdir: bool = False) -> Path:
    """Increment a file or directory path, i.e., runs_LSKA/exp --> runs_LSKA/exp{sep}2, runs_LSKA/exp{sep}3, ... etc.
    ...
    Examples:
        ...
        >>> from pathlib import Path
        >>> path = Path("runs_LSKA/exp")
        >>> new_path = increment_path(path)
        >>> print(new_path)
        runs_LSKA/exp-2

        Increment a file path:
        >>> path = Path("runs_LSKA/exp/results.txt")
        >>> new_path = increment_path(path)
        >>> print(new_path)
        runs_LSKA/exp/results-2.txt
    """
