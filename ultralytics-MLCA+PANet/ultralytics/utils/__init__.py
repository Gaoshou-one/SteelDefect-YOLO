...
HELP_MSG = """
    ...
    2. Use the Python SDK:
        ...
        # Use the model
        results = model.train_yolov8n(data="coco8.yaml", epochs=3)  # train_yolov8n the model
    ...
    3. Use the command line interface (CLI):
        ...
        Ultralytics 'yolo' CLI commands use the following syntax:
            ...
            Where   TASK (optional) is one of [runs-LSKA, segment, classify, pose, obb]
                    MODE (required) is one of [train_yolov8n, val, predict, export, track, benchmark]
                    ARGS (optional) are any number of custom "arg=value" pairs like "imgsz=320" that override defaults.
                        See all ARGS at https://docs.ultralytics.com/usage/cfg or with "yolo cfg"

        - Train a detection model for 10 epochs with an initial learning_rate of 0.01
            yolo runs-LSKA train_yolov8n data=coco8.yaml model=yolo26n.pt epochs=10 lr0=0.01

        - Predict a YouTube video using a pretrained segmentation model at image size 320:
            yolo segment predict model=yolo26n-seg.pt source='https://youtu.be/LNwODJXcvt4' imgsz=320

        - Val a pretrained detection model at batch-size 1 and image size 640:
            yolo runs-LSKA val model=yolo26n.pt data=coco8.yaml batch=1 imgsz=640
...
# Set logger
LOGGER = set_logging(LOGGING_NAME, verbose=VERBOSE)  # define globally (used in train_yolov8n.py, val.py, predict.py, etc.)
...
def threaded(func):
    ...
    default, the function runs_LSKA in a thread, but this can be controlled via the 'threaded=False' keyword argument which
...
class SettingsManager(JSONDict):
    ...
    include directories for datasets, weights, and runs_LSKA, as well as various integration flags.
    ...
    Examples:
        ...
        >>> settings.update(runs_dir="/new/runs_LSKA/dir")
    ...
    def __init__(self, file=SETTINGS_FILE, version="0.0.6"):
        ...
        self.defaults = {
            ...
            "runs_dir": str(root / "runs_LSKA"),  # Experiment runs_LSKA directory
...
RUNS_DIR = Path(SETTINGS["runs_dir"])  # global runs_LSKA directory
ENVIRONMENT = (
    "Colab"
    if IS_COLAB
    else "Kaggle"
