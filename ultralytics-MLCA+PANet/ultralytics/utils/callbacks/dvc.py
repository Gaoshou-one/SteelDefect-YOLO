...
def _log_images(path: Path, prefix: str = "") -> None:
    ...
    Examples:
        >>> from pathlib import Path
        >>> _log_images(Path("runs_LSKA/train_yolov8n/exp/val_batch0_pred.jpg"), prefix="validation")
    """
    if live:
        name = path.name
...
def _log_confusion_matrix(validator) -> None:
    ...
    if validator.confusion_matrix.task == "runs-LSKA":
        names += ["background"]
...
def on_pretrain_routine_end(trainer) -> None:
    ...
    _log_plots(trainer.plots, "train_yolov8n")
...
def on_fit_epoch_end(trainer) -> None:
    ...
    if live and _training_epoch:
        all_metrics = {**trainer.label_loss_items(trainer.tloss, prefix="train_yolov8n"), **trainer.metrics, **trainer.lr}
...
def on_train_end(trainer) -> None:
    ...
    """
    if live:
        # At the end log the best metrics. It runs_LSKA validator on the best model internally.
        all_metrics = {**trainer.label_loss_items(trainer.tloss, prefix="train_yolov8n"), **trainer.metrics, **trainer.lr}
        for metric, value in all_metrics.items():
            live.log_metric(metric, value, plot=False)
