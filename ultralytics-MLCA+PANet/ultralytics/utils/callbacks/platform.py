...
def _get_project_name(trainer):
    ...
    return project, slugify(str(trainer.args.name or "train_yolov8n"))
...
def on_pretrain_routine_start(trainer):
    ...
    # Build trainArgs - callback runs_LSKA before get_dataset() so args.data is still original (e.g., ul:// URIs)
    ...
    if response and response.get("modelId"):
        ...
        # Server returns actual slug (may differ from requested name due to auto-increment, e.g. "train_yolov8n" → "train_PANet")
...
def on_fit_epoch_end(trainer):
    ...
    project, name = _get_project_name(trainer)
    metrics = {**trainer.label_loss_items(trainer.tloss, prefix="train_yolov8n"), **trainer.metrics}
    ...
    def _send_and_check_cancel():
        """Send epoch_end and check response for cancellation (runs_LSKA in background thread)."""
