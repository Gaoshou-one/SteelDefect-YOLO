...
Commands:
    ...
    3. To start a local MLflow server:
        mlflow server --backend-store-uri runs_LSKA/mlflow
...
def on_pretrain_routine_end(trainer):
    ...
    Notes:
        MLFLOW_TRACKING_URI: The URI for MLflow tracking. If not set, defaults to 'runs_LSKA/mlflow'.
...
def on_train_epoch_end(trainer):
    """Log training metrics at the end of each train_yolov8n epoch to MLflow."""
    if mlflow:
        mlflow.log_metrics(
            metrics={
                **sanitize_dict(trainer.lr),
                **sanitize_dict(trainer.label_loss_items(trainer.tloss, prefix="train_yolov8n")),
            },
            step=trainer.epoch,
        )
