...
class ConfusionMatrix(DataExportMixin):
    ...
    Attributes:
        task (str): The type of task, either 'runs-LSKA' or 'classify'.
    ...
    def __init__(self, names: dict[int, str] = {}, task: str = "runs-LSKA", save_matches: bool = False):
        """Initialize a ConfusionMatrix instance.

        Args:
            names (dict[int, str], optional): Names of classes, used as labels on the plot.
            task (str, optional): Type of task, either 'runs-LSKA' or 'classify'.
    ...
    def tp_fp(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        return (tp, fp) if self.task == "classify" else (tp[:-1], fp[:-1])  # remove background class if task=runs-LSKA
