...
Usage:
    $ yolo mode=train_yolov8n model=yolo26n.pt data=coco8.yaml imgsz=640 epochs=100 batch=16
"""
...
class BaseTrainer:
    ...
    Attributes:
        ...
        epochs (int): Number of epochs to train_yolov8n for.
    ...
    Methods:
        train_yolov8n: Execute the training process.
        validate: Run validation on the val set.
        save_model: Save model training checkpoints.
        get_dataset: Get train_yolov8n and validation datasets.
    ...
    def train(self):
        ...
        # Run subprocess if DDP training, else train_yolov8n normally
        if self.ddp:
            # Argument checks
            if self.args.rect:
    ...
    def _build_train_pipeline(self):
        ...
        self.train_loader = self.get_dataloader(
            self.data["train_yolov8n"], batch_size=batch_size, rank=LOCAL_RANK, mode="train_yolov8n"
        )
    ...
    def _do_train(self):
        ...
        LOGGER.info(
            f"Image sizes {self.args.imgsz} train_yolov8n, {self.args.imgsz} val\n"
        ...
        self.optimizer.zero_grad()  # zero any resumed gradients to ensure stability on train_yolov8n start
        ...
        while True:
            ...
            for i, batch in pbar:
                ...
                # for/else: this block runs_LSKA only when the for loop completes without break (no OOM retry)
    ...
    def get_dataset(self):
        """Get train_yolov8n and validation datasets from data dictionary.
        ...
        try:
            ...
            if self.args.task == "classify":
                ...
                if "yaml_file" in data:
                    self.args.data = data["yaml_file"]  # for validating 'yolo train_yolov8n data=url.zip' usage
        except Exception as e:
    ...
    def get_dataloader(self, dataset_path, batch_size=16, rank=0, mode="train_yolov8n"):
        """Raise NotImplementedError (must return a `torch.utils.data.DataLoader` in subclasses)."""
        raise NotImplementedError("get_dataloader function not implemented in trainer")

    def build_dataset(self, img_path, mode="train_yolov8n", batch=None):
        """Build dataset."""
        raise NotImplementedError("build_dataset function not implemented in trainer")

    def label_loss_items(self, loss_items=None, prefix="train_yolov8n"):
            ...
            try:
                ...
                if ckpt_args.get("augmentations") is not None:
                    ...
                    LOGGER.warning(
                        ...
                        f"model.train_yolov8n(resume=True, augmentations={ckpt_args['augmentations']})"
                    )

            except Exception as e:
                raise FileNotFoundError(
                    "Resume checkpoint not found. Please pass a valid checkpoint to resume from, "
                    "i.e. 'yolo train_yolov8n resume model=path/to/last.pt'"
                ) from e
        self.resume = resume
    ...
    def _handle_nan_recovery(self, epoch):
        ...
        self._model_train()  # set model to train_yolov8n mode before loading checkpoint to avoid inference tensor errors
    ...
    def resume_training(self, ckpt):
        ...
        assert start_epoch > 0, (
            ...
            f"Start a new training without resuming, i.e. 'yolo train_yolov8n model={self.args.model}'"
        )
