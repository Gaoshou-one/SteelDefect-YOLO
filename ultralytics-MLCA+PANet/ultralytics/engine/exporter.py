...
class Exporter:
    ...
    def __call__(self, model=None) -> str:
        ...
        if fmt == "imx":
            if not self.args.int8:
                ...
                self.args.int8 = True
            if not self.args.nms and model.task in {"runs-LSKA", "pose", "segment"}:
                LOGGER.warning("IMX export requires nms=True, setting nms=True.")
                self.args.nms = True
            if model.task not in {"runs-LSKA", "pose", "classify", "segment"}:
                raise ValueError(
        ...
        if self.args.int8 and not self.args.data:
            self.args.data = DEFAULT_CFG.data or TASK2DATA[getattr(model, "task", "runs-LSKA")]  # assign default data
            LOGGER.warning(
        ...
        y = None
        for _ in range(2):  # dry runs_LSKA
    ...
    def export_coreml(self, prefix=colorstr("CoreML:")):
        ...
        if self.args.nms and self.model.task == "runs-LSKA":
