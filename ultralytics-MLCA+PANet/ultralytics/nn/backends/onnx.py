...
class ONNXBackend(BaseBackend):
    ...
    Loads and runs_LSKA inference with ONNX models (.onnx files) using either Microsoft ONNX Runtime with CUDA/CoreML
...
class ONNXIMXBackend(ONNXBackend):
    ...
    def forward(self, im: torch.Tensor) -> np.ndarray | list[np.ndarray] | tuple[np.ndarray, ...]:
        """Run IMX inference with task-specific output concatenation for runs-LSKA, pose, and segment tasks.
