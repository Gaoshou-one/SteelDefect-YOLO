...
class DetectionModel(BaseModel):
    ...
    def _predict_augment(self, x):
        """Perform augmentations on input image x and return augmented inference and train_yolov8n outputs.

        Args:
            x (torch.Tensor): Input image tensor.

        Returns:
            (tuple[torch.Tensor, None]): Augmented inference output and None for train_yolov8n output.
        """
        ...
        return torch.cat(y, -1), None  # augmented inference, train_yolov8n
...
class Ensemble(torch.nn.ModuleList):
    ...
    def forward(self, x, augment=False, profile=False, visualize=False):
        ...
        return y, None  # inference, train_yolov8n output
        ...
        if e.name == "models":
            raise TypeError(
                emojis(
                    ...
                    f"\nRecommend fixes are to train_yolov8n a new model using the latest 'ultralytics' package or to "
...
def guess_model_task(model):
    ...
    Returns:
        (str): Task of the model ('runs-LSKA', 'segment', 'classify', 'pose', 'obb').
    """
    ...
    if isinstance(model, (str, Path)):
        ...
        if "-seg" in model.stem or "segment" in model.parts:
            ...
            return "obb"
        elif "runs-LSKA" in model.parts:
            return "runs-LSKA"

    # Unable to determine task from model
    LOGGER.warning(
        "Unable to automatically guess model task, assuming 'task=runs-LSKA'. "
        "Explicitly define task for your model, i.e. 'task=runs-LSKA', 'segment', 'classify','pose' or 'obb'."
    )
    return "runs-LSKA"  # assume runs-LSKA
