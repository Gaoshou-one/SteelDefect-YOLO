...
class LSKA(nn.Module):
    """
    Large Separable Kernel Attention (大核可分离注意力)
    论文: https://arxiv.org/abs/2309.01439
    """

    def __init__(self, dim, kernel_size=21, *args, **kwargs):
        super().__init__()
        self.dim = dim
...
class RepBlock(nn.Module):
    """
    重参数化卷积块 (Reparam Block)
    用于 Gold-YOLO 的特征融合
    """

    def __init__(self, channels, out_channels=None, kernel_size=3, stride=1, *args, **kwargs):
        super().__init__()
        out_channels = out_channels or channels
        self.conv = nn.Conv2d(channels, out_channels, kernel_size, stride,
                              padding=kernel_size // 2, bias=False)
        self.bn = nn.BatchNorm2d(out_channels)
        self.conv_1x1 = nn.Conv2d(channels, out_channels, 1, stride, bias=False)
        self.bn_1x1 = nn.BatchNorm2d(out_channels)
        self.identity = nn.Identity() if channels == out_channels and stride == 1 else None
        self.act = nn.SiLU(inplace=True)

    def forward(self, x):
        out = self.bn(self.conv(x)) + self.bn_1x1(self.conv_1x1(x))
...
class Inject(nn.Module):
    ...
    def __init__(self, channels, *args, **kwargs):
        ...
        self.attn_conv = nn.Conv2d(channels * 2, channels, 1)
        self.attn_bn = nn.BatchNorm2d(channels)
    ...
    def forward(self, local_feat, global_feat):
        ...
        local_out = self.act(self.local_bn(self.local_conv(local_feat)))
        concat_feat = torch.cat([local_out, global_feat], dim=1)
        attn = self.sigmoid(self.attn_bn(self.attn_conv(concat_feat)))
        return local_out * attn + global_feat * (1 - attn)


class Inject_LAF(nn.Module):
    """增强型信息注入模块"""

    def __init__(self, channels, *args, **kwargs):
        super().__init__()
        self.inject = Inject(channels)
        self.laf_conv = nn.Conv2d(channels * 2, channels, 1)
        self.laf_bn = nn.BatchNorm2d(channels)
        self.act = nn.SiLU(inplace=True)

    def forward(self, feat, global_feat, adj_feat=None):
        enhanced = self.inject(feat, global_feat)
        if adj_feat is not None:
            if adj_feat.shape[2:] != enhanced.shape[2:]:
                adj_feat = F.interpolate(adj_feat, size=enhanced.shape[2:],
                                         mode='bilinear', align_corners=False)
            concat = torch.cat([enhanced, adj_feat], dim=1)
            return self.act(self.laf_bn(self.laf_conv(concat)))
        return enhanced
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, num_layers=1, *args, **kwargs):
        super().__init__()
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=dim, nhead=num_heads,
            dim_feedforward=dim * 4, batch_first=True, dropout=0.1
        )
...
class MLCA(nn.Module):
    """
    Mixed Local Channel Attention (混合局部通道注意力)
    同时融合局部特征与全局特征，增强通道信息交互
    """

    def __init__(self, channels, reduction=16, *args, **kwargs):
        super().__init__()
        self.channels = channels
        self.reduction = reduction

        # 局部和全局平均池化
        self.local_avg_pool = nn.AdaptiveAvgPool2d(1)
        self.global_avg_pool = nn.AdaptiveAvgPool2d(1)

        # 1D 卷积用于通道注意力
        reduced_channels = max(channels // reduction, 1)
        self.local_conv = nn.Conv1d(channels, reduced_channels, 1)
        self.global_conv = nn.Conv1d(channels, reduced_channels, 1)
        self.fc = nn.Conv1d(reduced_channels, channels, 1)
    ...
    def forward(self, x):
        ...
        # 局部特征
        local_feat = self.local_avg_pool(x).view(b, c)
        # 全局特征
        global_feat = self.global_avg_pool(x).view(b, c)

        # 计算注意力权重
        local_att = self.sigmoid(self.fc(self.local_conv(local_feat.unsqueeze(-1))).squeeze(-1))
        global_att = self.sigmoid(self.fc(self.global_conv(global_feat.unsqueeze(-1))).squeeze(-1))

        # 融合注意力
        att = (local_att + global_att) / 2
