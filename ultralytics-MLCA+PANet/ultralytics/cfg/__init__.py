...
MODES = frozenset({"train_yolov8n", "val", "predict", "export", "track", "benchmark"})
TASKS = frozenset({"runs-LSKA", "segment", "classify", "pose", "obb"})
TASK2DATA = {
    "runs-LSKA": "coco8.yaml",
...
CLI_HELP_MSG = f"""
    ...
    1. Train a detection model for 10 epochs with an initial learning_rate of 0.01
        yolo train_yolov8n data=coco8.yaml model=yolo26n.pt epochs=10 lr0=0.01
...
def get_save_dir(args: SimpleNamespace, name: str | None = None) -> Path:
    ...
    Examples:
        >>> from types import SimpleNamespace
        >>> args = SimpleNamespace(project="my_project", task="runs-LSKA", mode="train_yolov8n", exist_ok=True)
        >>> save_dir = get_save_dir(args)
        >>> print(save_dir)
        runs_LSKA/runs-LSKA/my_project/train_yolov8n
    """
    if getattr(args, "save_dir", None):
        ...
        if not Path(project).is_absolute():
            project = (ROOT.parent / "tests/tmp/runs_LSKA" if TESTS_RUNNING else RUNS_DIR) / args.task / project
...
def handle_yolo_solutions(args: list[str]) -> None:
    ...
    Notes:
        ...
        - Output videos are saved in 'runs_LSKA/solution/{solution_name}' directory
    ...
    if solution_name == "inference":
        ...
        if solution_name != "crop":
            ...
            save_dir.mkdir(parents=True, exist_ok=True)  # create the output directory i.e. runs_LSKA/solutions/exp
...
def entrypoint(debug: str = "") -> None:
    ...
    Examples:
        ...
        >>> entrypoint("train_yolov8n data=coco8.yaml model=yolo26n.pt epochs=10 lr0=0.01")
    ...
    if task:
        if task not in TASKS:
            if task == "track":
                LOGGER.warning(
                    f"invalid 'task=track', setting 'task=runs-LSKA' and 'mode=track'. Valid tasks are {list(TASKS)}.\n{CLI_HELP_MSG}."
                )
                task, mode = "runs-LSKA", "track"
            else:
    ...
    elif mode in {"train_yolov8n", "val"}:
