...
def check_det_dataset(dataset: str, autodownload: bool = True) -> dict[str, Any]:
    ...
    # Checks
    for k in "train_yolov8n", "val":
        if k not in data:
            if k != "val" or "validation" not in data:
                raise SyntaxError(
                    emojis(f"{dataset} '{k}:' key missing ❌.\n'train_yolov8n' and 'val' are required in all data YAMLs.")
                )
    ...
    for k in "train_yolov8n", "val", "test", "minival":
...
def check_cls_dataset(dataset: str | Path, split: str = "") -> dict[str, Any]:
    ...
    Returns:
        (dict[str, Any]): A dictionary containing the following keys:
            ...
            - 'train_yolov8n' (Path): The directory path containing the training set of the dataset.
    ...
    train_set = data_dir / "train_yolov8n"
    if not train_set.is_dir():
        LOGGER.warning(f"Dataset 'split=train_yolov8n' not found at {train_set}")
    ...
    nc = len([x for x in (data_dir / "train_yolov8n").glob("*") if x.is_dir()])  # number of classes
    names = [x.name for x in (data_dir / "train_yolov8n").iterdir() if x.is_dir()]  # class names list
    names = dict(enumerate(sorted(names)))

    # Print to console
    for k, v in {"train_yolov8n": train_set, "val": val_set, "test": test_set}.items():
    ...
    return {"train_yolov8n": train_set, "val": val_set, "test": test_set, "nc": nc, "names": names, "channels": 3}
...
class HUBDatasetStats:
    ...
    Args:
        ...
        task (str): Dataset task. Options are 'runs-LSKA', 'segment', 'pose', 'classify', 'obb'.
    ...
    Examples:
        ...
        >>> stats = HUBDatasetStats("path/to/coco8.zip", task="runs-LSKA")  # runs-LSKA dataset
    ...
    def __init__(self, path: str = "coco8.yaml", task: str = "runs-LSKA", autodownload: bool = False):
        """Initialize class."""
        path = Path(path).resolve()
        LOGGER.info(f"Starting HUB dataset checks for {path}....")

        self.task = task  # runs-LSKA, segment, pose, classify, obb
        if self.task == "classify":
            unzip_dir = unzip_file(path)
            data = check_cls_dataset(unzip_dir)
            data["path"] = unzip_dir
        else:  # runs-LSKA, segment, pose, obb
    ...
    def get_json(self, save: bool = False, verbose: bool = False) -> dict:
        ...
        for split in "train_yolov8n", "val", "test":
