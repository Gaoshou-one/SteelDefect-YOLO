...
class YOLODataset(BaseDataset):
    ...
    Examples:
        >>> dataset = YOLODataset(img_path="path/to/images", data={"names": {0: "person"}}, task="runs-LSKA")
        >>> dataset.get_labels()
    """

    def __init__(self, *args, data: dict | None = None, task: str = "runs-LSKA", **kwargs):
        """Initialize the YOLODataset.

        Args:
            data (dict, optional): Dataset configuration dictionary.
            task (str): Task type, one of 'runs-LSKA', 'segment', 'pose', or 'obb'.
    ...
    def get_labels(self) -> list[dict]:
        ...
        if len_segments and len_boxes != len_segments:
            LOGGER.warning(
                ...
                "To avoid this please supply either a runs-LSKA or segment dataset, not a runs-LSKA-segment mixed dataset."
            )
            for lb in labels:
                lb["segments"] = []
        if len_cls == 0:
...
class YOLOMultiModalDataset(YOLODataset):
    ...
    Examples:
        >>> dataset = YOLOMultiModalDataset(img_path="path/to/images", data={"names": {0: "person"}}, task="runs-LSKA")
...
class GroundingDataset(YOLODataset):
    ...
    Examples:
        >>> dataset = GroundingDataset(img_path="path/to/images", json_file="annotations.json", task="runs-LSKA")
        >>> len(dataset)  # Number of valid images with annotations
    """

    def __init__(self, *args, task: str = "runs-LSKA", json_file: str = "", max_samples: int = 80, **kwargs):
        """Initialize a GroundingDataset for object detection.

        Args:
            json_file (str): Path to the JSON file containing annotations.
            task (str): Must be 'runs-LSKA' or 'segment' for GroundingDataset.
            max_samples (int): Maximum number of samples to load for text augmentation.
            *args (Any): Additional positional arguments for the parent class.
            **kwargs (Any): Additional keyword arguments for the parent class.
        """
        assert task in {"runs-LSKA", "segment"}, "GroundingDataset currently only supports `runs-LSKA` and `segment` tasks"
