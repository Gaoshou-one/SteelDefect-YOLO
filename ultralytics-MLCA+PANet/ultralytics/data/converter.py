...
def convert_coco(
    ...
    for json_file in sorted(Path(labels_dir).resolve().glob("*.json")):
        ...
        if lvis:
            # NOTE: create folders for both train_yolov8n and val in advance,
            # since LVIS val set contains images from COCO 2017 train_yolov8n in addition to the COCO 2017 val split.
...
def convert_dota_to_yolo_obb(dota_root_path: str):
    ...
    The function processes images in the 'train_yolov8n' and 'val' folders of the DOTA dataset. For each image, it reads the
    ...
    for phase in {"train_yolov8n", "val"}:
...
async def convert_ndjson_to_yolo(ndjson_path: str | Path, output_path: str | Path | None = None) -> Path:
    ...
    Examples:
        ...
        >>> model.train_yolov8n(data="https://github.com/ultralytics/assets/releases/download/v0.0.0/coco8-ndjson.ndjson")
    """
    ...
    task = dataset_record.get("task", "runs-LSKA")
    ...
    if not is_classification:
        if "train_yolov8n" not in splits:
            raise ValueError(f"Dataset missing required 'train_yolov8n' split. Found splits: {sorted(splits)}")
        if "val" not in splits:
            train_records = [r for r in image_records if r.get("split") == "train_yolov8n"]
            if len(train_records) < 2:
                raise ValueError(
                    f"Dataset has only {len(train_records)} image(s) and no 'val' split. "
                    f"Need at least 2 images to auto-split into train_yolov8n/val."
                )
            ...
            LOGGER.warning(
                ...
                f"Auto-splitting {len(train_records)} images into {len(train_records) - val_count} train_yolov8n, {val_count} val. "
    ...
    async def process_record(session, semaphore, record):
        ...
        async with semaphore:
            ...
            if not image_path.exists():
                if _reuse:
                    for s in ("train_yolov8n", "val", "test"):
                        if s == split:
                            continue
                        candidate = (
