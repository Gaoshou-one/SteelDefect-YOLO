...
def test_predict_all_image_formats():
    ...
    # Collect all images from train_yolov8n and val
    expected = {"avif", "bmp", "dng", "heic", "jp2", "jpeg", "jpg", "mpo", "png", "tif", "tiff", "webp"}
    images = [im for im in (dataset_path / "images" / "train_yolov8n").glob("*.*") if im.suffix.lower().lstrip(".") in expected]
...
def test_results(model: str, tmp_path):
    ...
    for r in results:
        ...
        r.save_txt(txt_file=tmp_path / "runs_LSKA/tests/label.txt", save_conf=True)
        r.save_crop(save_dir=tmp_path / "runs_LSKA/tests/crops/")
...
def test_safe_download_unzips_local_path_archive(tmp_path):
    ...
    (dataset_dir / "images" / "train_yolov8n").mkdir(parents=True)
    (dataset_dir / "images" / "val").mkdir(parents=True)
    (dataset_dir / "labels" / "train_yolov8n").mkdir(parents=True)
    (dataset_dir / "labels" / "val").mkdir(parents=True)
    (dataset_dir / "data.yaml").write_text("path: .\ntrain_yolov8n: images/train_yolov8n\nval: images/val\nnames:\n  0: item\n")
...
def test_utils_files(tmp_path):
    ...
    exp_dir = tmp_path / "runs_LSKA" / "exp"
    exp_dir.mkdir(parents=True)
    assert increment_path(exp_dir) == tmp_path / "runs_LSKA" / "exp-2"
...
def test_yolo_world():
    ...
    model.train(
        data={"train_yolov8n": {"yolo_data": ["dota8.yaml"]}, "val": {"yolo_data": ["dota8.yaml"]}},
        epochs=1,
        imgsz=32,
        cache="disk",
        close_mosaic=1,
...
def test_grayscale(task: str, model: str, data: str, tmp_path) -> None:
    ...
    # remove npy files in train_yolov8n/val splits if exists, might be created by previous tests
    for split in {"train_yolov8n", "val"}:
